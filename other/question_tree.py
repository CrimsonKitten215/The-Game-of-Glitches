...
class GameInfo:
	...
	def get_gary_name_version(self, listy=False):
		if not listy:
			return self._gary_name_version
		return self._gary_name_version[listy]
		...
		while "V{" in new_text:
			...
			for i in range(start_i, len(new_text)):
				if new_text[i] == "}":
					...
					if variable[-1] == "]":
						proper_word = getattr(g, "get_" + variable[:variable.index("[")])()
						if isinstance(proper_word, list):
							proper_word = proper_word[int(variable[variable.index("["):])]
					else:
						if variable[-1] == ")":
							proper_word = getattr(g, "get_" + variable[:variable.index("(")])(variable[variable.index("(") + 1 : -2])
						else:
